...
- **In-Progress Draft & Recovery**
    - Automatic local saving of in-progress workout logs (routine or single exercise) if the app is closed or interrupted.
    - Resume prompt on app reopen, allowing users to continue or discard unfinished logs.
    - Drafts are only saved if there is meaningful progress (at least one valid set for an exercise or routine).
    - Drafts are deleted when the user finishes, switches workout modes, or clears all data.
    - Only one draft is ever stored, ensuring device storage remains clean and efficient.
    - Improved UI/UX for resuming, discarding, and cleaning up drafts.
