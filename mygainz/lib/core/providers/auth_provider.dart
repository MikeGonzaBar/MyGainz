...
import '../../features/auth/models/user_model.dart' as local_user;
...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> _initializeAuth() async {
    if (kDebugMode) print('Starting Firebase authentication initialization...');
    try {
      ...
      _firebaseAuth.authStateChanges().listen((firebase_auth.User? user) {
        if (kDebugMode) print('=== AUTH STATE CHANGE ===');
        if (user != null) {
          if (kDebugMode) print('User authenticated, loading data...');
          _printFirebaseUserInfo(user);
          _loadUserData(user.uid);
        } else {
          if (kDebugMode) print('User signed out or not authenticated');
          _currentUser = null;
          notifyListeners();
        }
        if (kDebugMode) print('========================');
  ...
  Future<void> _loadUserData(String uid) async {
    try {
      if (kDebugMode) print('Loading user data from Firestore for UID: $uid');

      final user = await _userFirestoreService.getUser(uid);

      if (user != null) {
        _currentUser = user;
        if (kDebugMode) print('User data loaded from Firestore: ${user.email}');
      } else {
        if (kDebugMode) print('No user data found in Firestore for UID: $uid');
      ...
      if (kDebugMode) print('Error loading user data from Firestore: $e');
  ...
  Future<void> _saveUserData(local_user.User user) async {
    try {
      ...
      if (kDebugMode) print('Error saving user data to Firestore: $e');
      notifyListeners();
    }
  }
  ...
  Future<bool> register({
    ...
    try {
      ...
      if (userCredential.user != null) {
        // Debug: Print all Firebase user information
        if (kDebugMode) print('=== REGISTRATION SUCCESS ===');
        ...
        _isLoading = false;
        notifyListeners();
        if (kDebugMode) print('User registered successfully: ${newUser.email}');
        return true;
      }
      ...
      _isLoading = false;
      notifyListeners();
      if (kDebugMode) print('Registration error: ${e.code} - ${e.message}');
      return false;
    } catch (e) {
  ...
  Future<bool> updateUserStats({
    ...
    try {
      ...
      _isLoading = false;
      notifyListeners();
      if (kDebugMode) print('User stats updated successfully in Firestore');
      return true;
    } catch (e) {
      _error = 'Stats update failed: ${e.toString()}';
      _isLoading = false;
      notifyListeners();
      if (kDebugMode) print('Stats update error: $e');
      return false;
    }
  }
  ...
  Future<bool> deleteAccount() async {
    ...
    try {
      ...
      _isLoading = false;
      notifyListeners();
      if (kDebugMode) print('Delete account error: ${e.code} - ${e.message}');
      return false;
    } catch (e) {
  ...
  Future<bool> changePassword({
    ...
    try {
      ...
      _isLoading = false;
      notifyListeners();
      if (kDebugMode) print('Change password error: ${e.code} - ${e.message}');
      return false;
    } catch (e) {
  ...
  Future<bool> updateProfile({
    ...
    try {
      ...
      _isLoading = false;
      notifyListeners();
      if (kDebugMode) print('Update profile error: ${e.code} - ${e.message}');
      return false;
    } catch (e) {
    ...
    try {
      ...
      for (final key in keys) {
        if (key.startsWith('user_data_')) {
          ...
          if (kDebugMode) print('Removed old data key: $key');
        }
      }

      if (kDebugMode) print('Cleanup of old authentication data completed');
    } catch (e) {
  ...
  void forceCompleteInitialization() {
    if (kDebugMode) print('Force completing initialization...');
    _isInitialized = true;
    _isLoading = false;
    notifyListeners();
  }
  ...
  Future<void> clearUserData() async {
    try {
      ...
      for (final key in keys) {
        if (key.startsWith('user_data_')) {
          await prefs.remove(key);
          if (kDebugMode) print('Removed old user data key: $key');
        }
      }

      if (kDebugMode) print('User data cleared from local storage');
    } catch (e) {
